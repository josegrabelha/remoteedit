...
function addLines(tab,lines,fromStderr){if(tab.paused){tab.pausedBuffer.push(...lines.map(raw=>({raw:raw,stderr:!!fromStderr})));if(tab.pausedBuffer.length>MAX_PAUSED){const extra=tab.pausedBuffer.length-MAX_PAUSED;tab.pausedBuffer.splice(0,extra);tab.discardedPaused+=extra;}renderStatus();return;}const activeTab=tab.id===state.activeTabId;const shouldFollow=activeTab&&tab.autoScroll&&isAtBottom()&&Number(tab.newLinesPending||0)===0;for(const raw of lines){tab.lines.push(parseLine(raw,tab,fromStderr));}if(tab.lines.length>MAX_LINES){const extra=tab.lines.length-MAX_LINES;tab.lines.splice(0,extra);tab.discardedOld+=extra;if(!tab.lines[0]||!tab.lines[0].marker){tab.lines.unshift({raw:'--- older loaded lines discarded due to line limit ---',marker:true});}}if(!activeTab){tab.newInactive+=lines.length;renderTabs();return;}computeMatches(tab);renderStatus();if(shouldFollow){renderContent();scrollBottom();}else{if(lines.length)tab.newLinesPending=Number(tab.newLinesPending||0)+lines.length;if(tab.lines.length<=MAX_RENDER){const previousScrollTop=els.content.scrollTop;renderContent();els.content.scrollTop=previousScrollTop;if(isAtBottom())tab.newLinesPending=0;}updateNewLinesButton(tab);}}
