...
export class RemoteEditSidebarController implements vscode.Disposable {
  ...
  private async showRemoteEntryProperties(item: RemoteEditSidebarItem | undefined): Promise<void> {
    ...
    try {
      ...
      const copyContent = [
        `${stats.type === 'directory' ? 'Directory' : 'File'} Properties`,
        '',
        `Name: ${name}`,
        `Remote path: ${item.remotePath}`,
        `Type: ${stats.type}`,
        `Size: ${stats.type === 'directory' ? '—' : this.formatBytes(stats.size)}`,
        `Modified: ${this.formatTimestamp(stats.modifyTime)}`,
        `Accessed: ${this.formatTimestamp(stats.accessTime)}`,
        `Connection: ${connection?.name || item.connectionId}`,
        `Host: ${connection ? `${connection.username}@${connection.host}:${connection.port}` : item.connectionId}`
      ].join('\n');
      const detail = [
        `Name: ${name}`,
        `Remote path: ${item.remotePath}`,
      ...
      if (choice === copyItem) {
        ...
        void vscode.window.showInformationMessage('Copied file properties.');
      }
    } catch (error) {
      this.showSidebarCommandError(error);
    }
